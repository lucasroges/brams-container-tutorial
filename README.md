...
Iniciaremos abordando como o BRAMS foi portado para a execução em *containers* e, na sequência, trataremos os dois casos de execução: *single-host* e *multi-host*. Por fim, mostraremos algumas maneiras de utilizar o *container* como um ambiente de testes para modificações na aplicação.
...
No Docker, uma imagem é um recurso que funciona como um *template*, com as aplicações e dados necessários para realizar as tarefas desejadas. Uma imagem é construída através de uma série de comandos a fim de instalar aplicações e armazenar dados para utilização. A utilização das imagens se dá através dos *containers*, que são instâncias das imagens.
...
Como essa etapa é recomendada para execução em *clusters* e esse ambiente não foi configurado para o tutorial, ela será tratada de maneira visual para servir como uma base para experimentos futuros dos usuários. Para esse exemplo, temos 4 *hosts*. Em relação a configuração dos *hosts*, não há nada em especial em relação ao Docker. Os únicos cuidados tomados foram configurar acesso SSH sem senha entre os *hosts* e conferir se todos estavam na mesma subrede. Ambos os critérios remetem a necessidades para execução *multi-host* de qualquer aplicação MPI, dentro ou fora de *containers*.
...
Caso haja algum problema com um dos serviços, as tarefas de cada um podem ser individualmente listadas, através da execução de `docker service ps <name>` no *host manager*.

![serviceps](figures/serviceps.png)

Erros na inicialização de alguma tarefa aparecerão na aba ERROR da saída acima.
...
Para tal, o código fonte pode ser editado externamente ao *container* e, ao concluir essa etapa, basta comprimir os arquivos necessários, como especificado no [Dockerfile](files/Dockerfile#L52).
...
É possível utilizar o *container* como um `bash`, para utilização interna. Para tal, é necessário fazer pequenas alterações no comando `docker run`. O comando é apresentado abaixo e as mudanças discutidas logo após.
...
Caso deseja compartilhar uma imagem para outros usuários, ou testes em outros ambientes, a maneira mais simples é utilizar o [Docker Hub](https://hub.docker.com/). Para tal, seguir a sequência de comandos:
